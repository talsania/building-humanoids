...
public:
  ReachyController(const rclcpp::Node::SharedPtr &node)
      : node_(node), move_group_(node, "dual_arm"),
        left_gripper_(node, "left_hand"),
        right_gripper_(node, "right_hand"), head_group_(node, "head"),
...
private:
  ...
  getCurrentPoses()
  {
    geometry_msgs::msg::Pose left_pose =
        move_group_.getCurrentPose("l_new").pose;
    geometry_msgs::msg::Pose right_pose =
        move_group_.getCurrentPose("r_new").pose;
                      ...
                      bool open)
  {
    ...
    if (open)
    {
      if (gripper.getName() == "left_finger_2")
      {
        values = {1.2};
      }
      else
